...
const TransferStatement: React.FC = () => {
  ...
  const { data, isError, isLoading } = useAccountStatementData(val);
        ...
        <div className="w-full lg:w-[50%] px-2 py-4 pt-[18px] my-[13px] lg:pt-0 lg:my-0" style={{ minHeight: "calc(-110px + 100dvh)" }}>
          <div className="flex flex-col h-full">
              ...
              <div className="w-full grid grid-cols-12 gap-3 p-3 bg-bg_Quaternary z-50 font-lato">
                <div className="col-span-10 px-2 flex items-center justify-between">
                  <div className="datepicker-container">
                    <DatePicker onChange={fromDate} className="w-full" />
                  </div>
                  <div className="datepicker-container">
                    <DatePicker onChange={EndDate} className="w-full" />
                  </div>
                </div>
                ...
                <div className="col-span-12 text-xs sm:text-sm flex items-center gap-2 justify-center">
                  {[7, 14, 28].map((val, i) => (
                    <button
                      key={i}
                      className="cursor-pointer active:scale-95 rounded-md px-3 border py-1 text-xs sm:text-sm whitespace-nowrap shadow bg-titleGrd font-medium text-text_Quaternary"
                      onClick={() => setDays(val)}
                    >
                      Last {val} Days
                    </button>
            ...
            <div className="w-full min-h-[calc(100dvh-150px)] flex items-center justify-center flex-col">
              ...
              <span className="text-text_Ternary font-normal text-center">No data found for selected date range...</span>
            </div>
          </div>
        </div>
        <RightDeskSidebar />
