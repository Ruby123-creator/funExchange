...
if (isError) return <p>Error fetching data</p>;
  return (
    ...
    <div className="flex items-start justify-start w-full lg:px-12 xl:px-20 xlg:px-24">
        ...
        <div className=' flex items-center flex-col lg:w-[50%] w-full  lg:pt-0 pt-[130px]'>
        <div className="w-full h-full" >
        {
            location.pathname !== "/" ? <div className="px-2 w-full lg:hidden">
                 <div
                className="flex flex-row font-manrope-regular items-center justify-start gap-2.5 relative">
                {
                    ["In Play","Competition"].map((item,i)=>{
                        return(
                            <div className="cursor-pointer flex flex-row items-center justify-center" onClick={()=>(setActive(item))}>
                            <span
                                className={`${active === item ? 'text-text_Quaternary bg-bg_Primary  bg-bg_Primary  border-primary':'text-text_Ternary bg-bg_Quaternary  border-ternary6'} text-sm border rounded-full px-[25px] py-2 text-[13px] md:text-sm lg:text-base font-bold leading-4 active:scale-95 block z-10`}>{item}</span>
                        </div>
                        )
                    })
                }
    ...
    {
        (active === "In Play" || location?.pathname === "/") ? <><InPlayEvents events={inPlayEvents}/>
        <UpcomingEvents events={upComingEvents}/></>: <NoFound text={"No Competitions Found for cricket..."} image={"/icons/cricket.svg"}/>
    }
